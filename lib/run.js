...
export default async function runWithTypeScript (config) {
  ...
  const { globalSetup, globalTeardown } = await loadGlobalSetup(config, prefix)
...
async function loadGlobalSetup (config, prefix) {
  if (!config['global-setup']) {
    return {}
  }

  const globalSetupFilePath = prefix ? join(prefix, config['global-setup']) : join(config.cwd, config['global-setup'])
  const mod = await import(globalSetupFilePath.replace(/\.ts$/, '.js'))
  const defaultExport = mod.default?.default ?? mod.default

  const { globalSetup, globalTeardown } = defaultExport

  return { globalSetup, globalTeardown }
}
