...
export default async function runWithTypeScript (config) {
  ...
  const { globalSetup, globalTeardown } = loadGlobalSetup(config, prefix)
...
function loadGlobalSetup (config, prefix) {
  if (!config['global-setup']) {
    return {}
  }

  const globalSetupFilePath = prefix ? join(prefix, config['global-setup']) : config['global-setup']
  const _require = createRequire(config.cwd)
  const mod = _require(globalSetupFilePath.replace(/\.ts$/, '.js'))

  const { globalSetup, globalTeardown } = mod.default
